...
- 自动将超长路径换行至 `*INCLUDE_PATH` 和 `*INCLUDE_PATH_RELATIVE` 的续行
...
- 高级 PDF 手册关键字回溯匹配（完全支持以 `/` 分隔的书签项）
- 交互式悬停卡片：关键字和字段的 Hover 提示卡片内提供了直达 LS-DYNA PDF 手册对应页码的链接
- 悬停卡片采用转置的 HTML 网格表格布局，包含主题实线边框、列分隔符，并通过动态徽章样式与主题色高亮活动字段
- `openManual` 命令：轻松跳转到指定的手册页面；若未找到匹配项，底部手册栏将自动隐藏
...
- 为悬停提示、侧边栏标题、提示和对话框等 UI 提供多语言（中/英文）本地化支持
...
- 阻止自动缩进，并在按下回车键时清除自动复制的尾随空格，以保持严格的数据对齐
